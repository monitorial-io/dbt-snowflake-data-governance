{% macro unset_column_projection_policy(materialization, model_schema, model_name, column_name) %}
    {%- call statement('set_statement', fetch_result=True) -%}
        alter {{materialization}} {{model_schema|upper}}.{{model_name|upper}} modify column {{column_name}} unset projection policy;
    {%- endcall -%}
