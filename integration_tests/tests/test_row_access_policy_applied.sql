-- Test that the row access policy was applied to the model
-- Returns rows if the expected policy is missing (test fails)
-- depends_on: {{ ref('test_row_access_policy_model') }}
{% set model_schema = target.database | upper ~ '.' ~ target.schema | upper %}

with expected_policies as (
    select 'RESTRICT_ROWS' as policy_name
),
actual_policies as (
    select upper(POLICY_NAME) as policy_name
    from table(information_schema.policy_references(
        ref_entity_name => '{{ model_schema }}.TEST_ROW_ACCESS_POLICY_MODEL',
